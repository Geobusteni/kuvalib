...
erDiagram
    ...
    Showcase {
        ...
        boolean musicAutoplay "default false; meaningless when autoplay is true"
        json headingSizes "H1-H6 default px sizes"
        json textSizes "small..huge default px sizes"
        string headingFont "Google Font name; null = app default"
        string textFont "Google Font name; null = app default"
        json colorPresets "admin palette; null = DEFAULT_COLOR_PRESETS"
...
other block field, since it's a property of that image, not of the page — and so do a group's
`shadow`/`shadowColor` and a Headline/Text block's `bold`/`italic`/`underline`.
...
| Lightbox photo prefetched on dialog mount (the lightbox no longer offers a download; the prefetch now only serves single-photo selections), keyed by photo id | `navigator.share()` needs a fresh user gesture; fetching the photo inside the click handler risked losing that window on slow connections. Scoped to the single-photo case (the lightbox's only usage) so multi-select downloads don't eagerly fetch photos a user may not pick |
...
| Expiry strip and lightbox overdraw below the viewport instead of relying on `bottom:0`/`h-dvh` | On phones the URL bar collapses while scrolling; a `position: fixed` box sized to the layout viewport (and `h-dvh`, which changes mid-animation) can trail the true bottom edge for a few frames, leaving a gap through which the page shows. Sizing tricks cannot be made exact across browsers, so the strip paints an `after:` skirt (`top-full h-screen`, same colour) and the lightbox's black backdrop extends `-bottom-[100vh]`; the gap is filled with the same colour. `backdrop-blur` was dropped from the strip (a pseudo-element cannot extend it and it lags on scroll). The lightbox root is plain `fixed inset-0` (no `h-dvh`). Verified by simulating a 60px lag with headless Chrome; real toolbar animation cannot be emulated. **Second and third pass (the gap persisted in Chrome for iPhone, not Safari):** a `visualViewport`-based pin (`usePinToVisualBottom`) was tried and reverted — Chrome iOS reports viewport numbers that do not match where `fixed` boxes sit, so it made the strip vanish while scrolling. The robust fix is to remove the cause: on touch devices (`@media (pointer: coarse)`) the gallery scrolls inside a `fixed inset-0 overflow-y-auto` box (`Gallery.tsx`) instead of the page, so the browser toolbar never collapses or expands and `fixed bottom-0` is reliable; desktop keeps page scrolling. `viewport-fit=cover` stays removed, and the strip's skirt is opaque black. Trade-off: the toolbar no longer hides while scrolling on phones. Checked with the touch rules forced on: the page does not scroll, the header stays pinned, the strip sits on the bottom edge, the footer buttons clear it and the lightbox fills the full height; real Chrome iOS could not be run |
| Lightbox swipe down/up follow the finger via direct style writes, axis locked in `useGestures` | State per pointermove would re-render the viewer every frame. `useGestures` locks horizontal vs vertical ~10px in and reports vertical drag/release (with velocity); `PhotoViewer` writes transform/opacity on refs and settles with a CSS transition (0ms under `prefers-reduced-motion`, drag still tracks). Close threshold 25% of viewport height or >0.5px/ms; the sheet mirrors it. The sheet does not exist from `sm` up (display none), so swiping up there is inert |
| Lightbox has no per-photo download and no `D` shortcut | Clients took the single-photo icon for "download the gallery". The swipe-up sheet only held that button plus Cancel, so it now holds Cancel alone (kept, with its animation, pending an owner decision) |
...
| Page dots are laid out horizontally with a fixed `gap`, not `justify-between` | `justify-between` needs a fixed-width container to spread items across and looks broken with one or two pages (they'd sit at the container's opposite corners); a fixed gap between 44×44 touch targets scales correctly regardless of page count |
| Button blocks gained the same `borderStyle`/`borderWidth`/`borderColor` fields as Image blocks (`BorderField` reused as-is) | No new type needed — the fields already existed on `Block` for images; an explicit border on a button now wins over the old implicit look (a 1px outline on "secondary", none on "primary"), which stays as the fallback so existing buttons don't change |
| `Showcase.colorPresets` (admin palette) is offered as quick-pick swatches under every custom-colour `<input type="color">` in the builder, rather than replacing those inputs | The request was "too many colour pickers," not "no colour picker" — an arbitrary one-off colour must still be reachable. Presets make the common case (reusing a brand colour) one click instead of reopening the native picker each time, without removing the escape hatch |
| The theme CSS vars (scoped to the whole builder grid — see the earlier `--sc-accent` row) also make the *default* background swatches (surface/deep/accent) resolve inside `AlbumSettingsDialog`'s Page-dot preset row and everywhere else in the panel, not just `BackgroundField` | Same latent bug, different symptom — any `var(--sc-*)` reference outside `Canvas`'s subtree was silently blank before that fix; worth calling out again since colour presets made every one of these pickers visible at once |
| `Showcase.autoplay` default flipped from `false` to `true` (schema default only — existing rows keep their stored value) | The user wants new showcases to autoplay out of the box; changing only the column default (not backfilling existing rows) respects a photographer's own choice on showcases they already configured |
| Clicking a page in the rail always deselects the current block (`actions.selectNode()` before `switchPage`), even when re-clicking the already-active page | `switchPage` no-ops when the page id is unchanged, so re-clicking the current page while a block was selected previously did nothing — the settings panel stayed on the block instead of falling back to page settings |
| `BlockTree`'s Craft `useEditor` collector returns Craft's own `ids` array reference and reads per-row data (`query.node(id).get()`) in the render body, not inside the collector | An earlier version built a fresh `rows` array via `.map()` inside the collector on every collect — a fresh array recreated on each store notification, needlessly triggering Craft's `useCollector` `setState` far more than the underlying data actually changed, which surfaced as a `setState`-during-render warning in the console. Matches the pattern `SettingsPanel` already used for the selected block |
| The gallery hides like/dislike/comment once a showcase exists (`feedbackEnabled = project.feedbackEnabled && !showcase` in `app/(gallery)/g/[slug]/page.tsx`), rather than a separate DB flag | The project's own `feedbackEnabled` column is left untouched, so feedback comes back automatically if the showcase is deleted — no extra state to keep in sync, and the showcase's existence is already fetched on that page for the "View showcase" link |
| Google Fonts are a curated allowlist (`GOOGLE_FONTS` in `showcase-blocks.ts`), not a free-text family name | `sanitizeFontFamily` rejects anything not in the list, so `headingFont`/`textFont` can go straight into a `font-family` CSS value and a Google Fonts stylesheet URL with no injection risk. A fixed list also keeps the picker to faces that actually suit a photography showcase rather than every font Google hosts |
| Font choice is album-wide (`Showcase.headingFont`/`textFont`), not per-block | The request was one font for all Headlines and one for all Text blocks, not per-block typography — matches how Heading/Text *sizes* already work (album defaults, no per-block font override even though sizes do have one) |
| Undo/redo uses Craft.js's own history (`actions.history.undo()/redo()`, `query.history.canUndo()/canRedo()`) rather than a bespoke implementation | Craft already tracks every `setProp`/`move`/`add`/`delete` action for the current page's node tree via `immer` patches — wiring up the existing mechanism (toolbar buttons + a window `keydown` listener for Ctrl/Cmd+Z) took a fraction of the work a parallel undo stack would have, at the cost of it covering only the active page's block edits, not page add/delete or Album settings (both live in the zustand store, outside Craft's tree) |
...
| Showcase pixel sizes are fluid: `fluidPx(n)` = `calc(n * min(1px, 0.078125cqw))` (authored `n` px at a 1280 px frame, linear below, never above); font sizes are `max(min(n, floor)px, fluidPx(n))` with floors H1 22 / H2 20 / H3 18 / H4 16 / H5 15 / H6 14 / Text 14 / Button 14 (`lib/showcase-theme.ts`). The frame (viewer `.sc-page`, builder canvas, thumbnail preview box) is `container-type: inline-size` | Fixed px text did not shrink on phones. Container query units need no JS measurement and resolve per frame, so the same formula makes the builder canvas (smaller than the viewer) WYSIWYG-proportional. 1280 is a typical laptop frame, so existing albums look unchanged on desktops. `min(1px, …)` keeps it capped at the authored size and works for negative shadow offsets (a number times a length), which `min(Npx, …)` would not. Border widths stay in px so hairlines survive |
| The viewer page frame fills its stage, except when the stage (`.sc-stagebox`, `container: size`) is under 1024 px wide and taller than 16:10: then `.sc-stage-frame` is `min(100cqw, 160cqh)` × `min(100cqh, 62.5cqw)`, centred, over an album-background band, and sets `--sc-floor: 0` (all in a `@container` rule in `app/globals.css`, no JS). `fluidFontPx` multiplies its floor by `var(--sc-floor, 1)`. The prev/next arrows stay vertically centred on the stage (`top: (100% − --expiry-bar-h) / 2`), which is also the letterboxed frame's centre; an earlier version parked them in the band below the frame and they looked misaligned on portrait phones. The stage is `z-0` (own stacking context) so the thumbnail rail (`z-10`) always paints above slideshow elements, below the top bar (`z-20`) and the expiry strip (`z-[70]`) | Block geometry is percentages of the frame, so a 3-column photo page on a phone became slivers; panning sideways (the previous decision, frame `max(100%, 640px)`) hid content and needed a hint. Letterboxing keeps the exact desktop layout in miniature, like video. Text floors exist so text does not vanish on a small *filling* frame; in a scaled frame they would make text overflow its block, so they are dropped there. Landscape phones (16:10 or wider) and 1024 px+ windows keep filling, where floors matter little and screen area is not wasted. Container query units make the switch a pure-CSS decision on the real viewer area (minus the expiry strip), not the window |
| Slideshow autoplay is one `setTimeout` per settled page, re-armed by any change of page, transition phase or manual navigation counter; `step` reads a phase ref and never runs side effects inside a state updater | The old `setInterval` kept running through manual navigation, so a page could change again moments after the visitor's own click. Side effects inside a `setState` updater also ran twice under React Strict Mode (dev), advancing two pages per step |
...
| The gallery toolbar is `sticky` in normal flow and wraps (title `flex-1 basis-32 truncate`, actions `flex-wrap`), not `fixed` at a fixed height; the gallery route pads with `env(safe-area-inset-*)` (it no longer sets `viewport-fit=cover`; see the expiry-strip row) | A fixed 56px bar cannot grow when it wraps to two rows at 240px, so it would cover photos; in flow it pushes content down by exactly its own height. Without `viewport-fit=cover` the insets are 0 and the browser keeps content clear of a notch itself |
