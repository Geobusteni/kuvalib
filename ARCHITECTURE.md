...
| The viewer page frame is `max(100%, 640px)` wide inside a natively scrollable `.sc-pan` container (starts at the left edge, where text begins, so headlines are not cropped mid-word; no JS gesture code, `overscroll-behavior-x: contain`); the nav buttons, controls and rail sit outside it, so they stay fixed while it pans. The frame's `overflow: hidden` wrapper stops a page-turn transform from growing the scroll area | Block geometry is percentages of the frame, so a 3-column photo page on a 360 px phone became unreadable slivers. Keeping the layout but never narrower than 640 px (about a 3-column page at 166 px per narrow column) was the owner's choice over reflowing. The viewer has no swipe-to-turn gesture, so there is nothing to disambiguate from native panning |
