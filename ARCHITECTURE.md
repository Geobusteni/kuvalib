...
| Expiry strip and lightbox overdraw below the viewport instead of relying on `bottom:0`/`h-dvh` | On phones the URL bar collapses while scrolling; a `position: fixed` box sized to the layout viewport (and `h-dvh`, which changes mid-animation) can trail the true bottom edge for a few frames, leaving a gap through which the page shows. Sizing tricks cannot be made exact across browsers, so the strip paints an `after:` skirt (`top-full h-screen`, same colour) and the lightbox's black backdrop extends `-bottom-[100vh]`; the gap is filled with the same colour. `backdrop-blur` was dropped from the strip (a pseudo-element cannot extend it and it lags on scroll). The lightbox root is plain `fixed inset-0` (no `h-dvh`). Verified by simulating a 60px lag with headless Chrome; real toolbar animation cannot be emulated. **Second pass (the gap persisted on a real iPhone):** overdraw alone is not enough, because the skirt was translucent and the strip still trailed. `hooks/usePinToVisualBottom.ts` now measures the difference between the element's bottom edge and the visual viewport's bottom (`visualViewport.offsetTop + height`) on every scroll/resize and closes it with a `translateY` on the strip (`translate`) or by growing the lightbox root's `bottom` (`extend`), for 500 ms after the last event so it settles; it does nothing without the Visual Viewport API or while pinch-zoomed. The skirt is now opaque black. `viewport-fit=cover` was removed from the gallery route, since it makes iOS anchor bottom elements to the full screen under the toolbar. Checked by faking `visualViewport` (+40 px / −30 px): the strip's bottom edge followed exactly, and stayed put in the normal state |
