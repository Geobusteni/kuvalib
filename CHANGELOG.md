...
- **Blocks now stick to their neighbours while you drag.** When a block's edge
  comes close to the opposite edge of another block at the same level (both
  top-level, or both inside the same group), it is held flush against it.
  Push it about 12 px further into the neighbour and it lets go, so you can
  still overlap blocks deliberately. A block inside a group is never pulled
  toward blocks outside that group.

### Changed

- Snapping to other blocks is no longer only applied when you let go; the
  page-edge and centre alignment on drop is unchanged.
