...
export default function ShowcaseManager({ projectId, showcase }: Props) {
  const t = useTranslations('showcaseBuilder.showcaseManager')
      ...
      {showcase ? (
        <div className="mt-3 flex flex-wrap items-center gap-3">
          <span className="text-sm text-zinc-700 dark:text-zinc-300">
            {t('pageCount', { count: showcase.pageCount })}
          </span>
          <a
          ...
          {confirmingDelete ? (
            <span className="flex items-center gap-2 text-sm">
              <span className="text-zinc-600 dark:text-zinc-400">{t('confirmDelete')}</span>
              <button
                onClick={remove}
                disabled={busy}
