...
// On phones the browser toolbar collapses while scrolling and a fixed box can trail the real
// bottom edge, so usePinToVisualBottom shifts the strip onto the visible bottom, and the solid
// after: skirt below it hides any remaining gap instead of letting the page show through. No
// backdrop-filter: it cannot be extended by a pseudo-element and lags during scroll.
export default function ExpiryStrip({ expiresAt }: { expiresAt: string }) {
  const t = useTranslations('expiry')
  const format = useFormatter()
  const ref = useRef<HTMLDivElement>(null)
  const [days, setDays] = useState<number | null>(null)
  usePinToVisualBottom(ref, 'translate')
