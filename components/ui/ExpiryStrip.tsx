...
export default function ExpiryStrip({ expiresAt }: { expiresAt: string }) {
  ...
  return (
    <div
      ref={ref}
      className="pointer-events-none fixed inset-x-0 bottom-0 z-[70] bg-black/80 pb-[max(0.5rem,env(safe-area-inset-bottom))] pl-[max(1rem,env(safe-area-inset-left))] pr-[max(1rem,env(safe-area-inset-right))] pt-2 text-center text-xs leading-snug backdrop-blur-sm"
    >
